...
async function generateDayEndReport(isForce = false) {
    ...
    const expected = totalFloat + totalCashSales;
    ...
    printArea.innerHTML = `
        ...
        <div style="font-size: 9pt; margin-bottom: 3mm;">
            ...
            <div style="display: flex; justify-content: space-between; border-top: 1px dashed black; padding-top: 1mm; font-weight: bold;">
                ...
                <span>Rs ${((m.float || 0) + (m.cashSales || 0)).toLocaleString()}</span>
            </div>
...
function generatePDF(m, sales) {
    ...
    doc.autoTable({
        ...
        body: [
            ...
            ["Expected Cash", `Rs ${(m.float + m.cashSales).toLocaleString()}`],
...
async function sendDayEndEmail(metrics) {
    try {
        ...
        const templateParams = {
            ...
            report_date: today,
            float_cash: `Rs ${metrics.float.toLocaleString()}`,
            cash_sales: `Rs ${metrics.cashSales.toLocaleString()}`,
            credit_sales: `Rs ${metrics.creditSales.toLocaleString()}`,
            expected_cash: `Rs ${(metrics.float + metrics.cashSales).toLocaleString()}`,
            actual_cash: `Rs ${metrics.cashInHand.toLocaleString()}`,
            variance: `Rs ${metrics.variance.toLocaleString()}`,
            total_revenue: `Rs ${metrics.totalSales.toLocaleString()}`,
            estimated_profit: `Rs ${metrics.profit.toLocaleString()}`,
